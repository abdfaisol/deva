namespace Deva
{
    ...
    public class HttpSession : TcpSession
    {
        ...
        /// <param name="request">HTTP request</param>
        protected virtual async Task OnReceivedRequestAsync(HttpRequest request) { }
        ...
        private async Task OnReceivedRequestInternal(HttpRequest request)
        {
            ...
            // Process the request
            await OnReceivedRequestAsync(request);
        }
        public HttpResponse FileWatcher(string path = "") => FileWatcher(path);
        //public HttpResponse FileWatcher(ReadOnlySpan<char> path)
        //{
        //    FileSystemWatcher watcher = new FileSystemWatcher();
        //    watcher.Path = Path.GetDirectoryName("/");
        //    watcher.Filter = Path.GetFileName("root.txt");
